...
design-effect range; the more restrictive PrEP-only indexing shown in
Figure~\ref{fig:detect} gives RR~$\approx$~14--64, which overstates infeasibility.
Either way it is far above any plausible effect. Detection appears in a single
...
RR~$\approx$~14--64 to move a population rate (PrEP-only indexing shown; the
combined PrEP+PLWH primary tightens this to RR~$\approx$~6--28, still far right of
the band). The signal is above the line while the systems built to follow it are
not.}
\label{fig:detect}
\end{figure}
