...
% ---------- TITLE PAGE ----------
\begin{center}
{\Large\bfseries Can current surveillance follow doxy-PEP-associated \Sa{}
resistance? A multi-stream evidence audit}\\[1.2em]
\end{center}
...
Tel: +1-000-000-0000. E-mail: acdemidont@nyxdynamics.org\\
...
(95\% CI 1.42--10.68; 68/393 versus 5/163), with no effect on colonisation
clearance. Yet 0 of 6 outbreak-matched governmental guidelines (0 of 10 coded)
...
(RR~$\approx$~14--64) to move a rate.
...
and carries mobile tetracycline resistance determinants.\cite{grossman2016}
Doxycycline is one of a limited number of oral options for outpatient skin and
soft-tissue infection, a choice often made empirically before any methicillin
result returns,\cite{stevens2014,liu2011} so tetracycline susceptibility can
matter clinically before methicillin status is known.
...
highest-rated.\cite{dona2026} The trial literature has now answered a version of
...
al.}\cite{soge2025} (tetracycline-resistant \sa{} colonisation, 18\% versus 8\%
...
participant-level time-to-event test itself; we cite that randomised result
...
counsels on or measures the organism. A blinded second coder double-coded, with
per-field Cohen's $\kappa$. An independence check separated genuinely independent
declines from documents that defer to, or are adapted from, one another.
...
to move a population rate. The dilution fraction is
$f=\phi\,(\text{male-PrEP rate}/10^{5})\,\text{uptake}\,\kappa$,
population-independent because it tracks exposure density, not headcount, and the
minimum detectable change in a proportion near baseline $R_0$ inverts to
$\mathrm{RR_{needed}}=1+\mathrm{MDE}/(f R_0)$. Parameters were bounded to favour
detection (so a failed gate is robust); full grids and the metropolitan
break-even are in the Supplementary data.

% ---------- RESULTS ----------
\section*{Results}

\subsection*{Stream A: the trials resolved the signal but reported it unstably}
...
1.01).\cite{luetkemeyer2025} The 2023 interim, by contrast, read a cross-sectional
resistance fraction as reassurance and was underpowered for any externally
...
data that could reconcile these first-party reports are not
released.\cite{luetkemeyer2023} Even the intensively surveilled gonococcus was
...
JAMA Synopsis\cite{flores2025} & 5\%$\rightarrow$16\% & Within arm, among \sa{}-isolated & Carrier-conditioned percentage; raw counts omitted\\
\bottomrule
\end{tabularx}
\end{table}

\subsection*{Stream B: guidance names the risk but instruments nothing}
...
US jurisdictions) the result is identical: 0 instrument the bystander. Inter-coder
agreement was 92\%; the single field with meaningful disagreement ($\kappa=0.58$)
is reported as such. Providers are nonetheless required to counsel patients about
resistance in commensal organisms---a harm no system measures.

\subsection*{Stream C: no deployed system links exposure to the phenotype}
...
more isolates. Under realistic exposure density and isolate volumes the
within-exposed effect required to move a population rate runs to
RR~$\approx$~14--64 across the design-effect range---far above any plausible
effect (Figure~\ref{fig:detect})---and detection appears only in a best-case cell
that compounds four simultaneously implausible assumptions (RR~$\approx$~1.93, and
failing on any one assumption alone). The metropolitan break-even requires a
male-PrEP density 2.1--33 times the densest US geography, or roughly 325\% of
adult males on PrEP under realistic isolate volumes. The only configuration that
restores sensitivity is to sample \sa{} directly from doxy-PEP-exposed persons,
which converts surveillance into a sentinel cohort.
...
RR~$\approx$~14--64 to move a population rate. The signal is above the line while
the systems built to follow it are not.}
\label{fig:detect}
\end{figure}
...
\subsection*{The literature mirrors the instrumentation}
...
Table~\ref{tab:audit} summarises the three streams against the yardstick.
...
C. Surveillance & Is exposure linked to phenotype? & No---0/12 systems; RR$_{\text{needed}}\approx$14--64 to detect\\
\bottomrule
\end{tabularx}
\end{table}
...
read a cross-sectional fraction as reassurance; the 2024 guideline appended a call
...
\subsection*{The phenotype axis is the wrong one for the clinical question}
Staphylococcal surveillance is standardised around methicillin at every tier,
...
doxycycline resistance without resolving which.\cite{liu2011,grossman2016} A
surveillance variable can remain valid for the question it was designed to answer
while being insufficient for the one now imposed on it. We do not forecast which
...
denominator; resistance reported per carrier and by mechanism (\emph{tet}(M)
versus \emph{tet}(K)) rather than as a pooled tetracycline-class rate; dosing
frequency rather than prescription status; a feed carrying \sa{} tetracycline
...
geography; and release of the individual-level trial data. None is exotic; none
...
surveillance architecture standardised around methicillin while the tetracycline
...
The coupling has a distributive consequence, and it is a measurement asymmetry
before it is a moral one. The benefit is measured in an identified exposed
population, while any resistance externality would fall on the outpatient with a
skin infection and, at the sharp end, the dialysis, surgical or oncology patient
for whom an oral anti-staphylococcal agent is no convenience---none party to the
prescribing decision or counted by any deployed
system.\cite{montoya2025affected} The evidence architecture permits one side of
the ledger to be estimated far more completely than the other; a negative
externality no instrument can price is not thereby small, only unweighable.
...
no deployed system can determine. The trial's own randomised result is
significant and we accept rather than dispute it; our earlier aggregate
reconstruction of the interim is retired in its favour. Colonisation is not
...
and mechanism remain open. Consistent with a falsification standard set in
advance, the inheritance claim fails if a doxy-PEP guideline comes to require
