...
need to see (RR~14--64, Results) exceeds both the cross-sectional benchmark and
...
$f=\phi\,(\text{male-PrEP rate}/10^{5})\,\text{uptake}\,d\,\kappa$, where $d$ is
the share of users taking $>$3 doses/month (the subgroup carrying the benchmark
effect) and $\kappa$ the isolate-enrichment factor; $f$ is population-independent
...
adult males on PrEP under realistic isolate volumes. Detectability is itself
assumption-dependent, in a way that is part of the finding: under the most
optimistic panel-power treatment (every geography-year isolate independent) the
grid-median $\mathrm{RR_{needed}}$ falls to about 2.9 and 25 of 135 parameter
cells become nominally detectable---but only by relaxing the realistic
under-sampling and dose-threshold assumptions, and the realistic cell still
requires RR~14--64. The verdict thus turns on parameters no deployed system
reports---exposure density, isolate enrichment and dosing frequency---which is the
measurement gap in miniature. The only configuration that restores sensitivity is
to sample \sa{} directly from doxy-PEP-exposed persons, which converts
surveillance into a sentinel cohort.
