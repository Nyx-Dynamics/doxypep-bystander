...
pass; two separate blinded LLM coding passes then coded each document under
separate prompts against the prespecified schema, with disagreements resolved by
the author against the source. Per-field agreement is reported as raw concordance:
agreement was 92\%, and the single field with meaningful disagreement
($\kappa=0.58$, context only) is reported unrounded. Because the passes share a
model family their errors are not independent (main-text Limitations).
