...
the guidance corpus (Stream B), two separate blinded LLM coding passes---run
through different interfaces (a command-line agent and a chat interface) but the
same model family---coded each document against the schema, and disagreements were
...
and mechanism remain open. Our coding carries two acknowledged asymmetries. The guidance corpus was coded by
two machine passes run through different interfaces---a command-line agent and a
chat interface---but the same model family, so their errors are not independent;
the 92\% concordance measures the reproducibility of the coding protocol, not its
accuracy. The surveillance enumeration was coded by a single pass. Consistent with a
