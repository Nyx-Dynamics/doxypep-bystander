...
(Table~\ref{tab:dose}). Values regenerate via
\texttt{dilution\_plwh.dose\_sensitivity}.

\begin{table}[h]
\centering
\caption{Dose-threshold sensitivity of $\mathrm{RR_{needed}}$ (combined PrEP+PLWH,
2024). Realistic cell: $\kappa=0.5$, uptake 35\%, $R_0=10\%$, smallest $N$. Best
cell: every generous assumption maxed. $d=0.5$ is the primary specification.}
\label{tab:dose}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
$R_0=10\%$, $N=100\,000$/state-year, 2024). Every cell exceeds the more
conservative cross-organism benchmark (RR~1.42; minimum $\approx$1.9); the
\sa{}-matched benchmark is 2.25.}
\end{figure}
...
density (Washington, D.C., 6\,406/100\,000 adult males, 2024), against the
cross-organism benchmark (RR 1.42). $1\times$ is already the national maximum;
the best cell needs $\approx$2$\times$ it.}
\end{figure}
