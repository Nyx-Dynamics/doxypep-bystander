...
\large Can current surveillance follow doxy-PEP-associated \Sa{} resistance?
A multi-stream evidence audit}
...
archived with a citable DOI. US spelling is used (at author request); \textit{JAC}
nomenclature conventions are otherwise followed and MICs are expressed in mg/L.
...
$E$ exposure & Randomized doxy-PEP & Prescribing context & PrEP proxy only\\
$P$ phenotype & Incident doxy-R (present) & Not measured (absent) & Methicillin axis (wrong unit)\\
$D$ denominator & Randomized participants & None for \sa{} & Population, but methicillin\\
...
colonization across independent MSM cohorts was estimated by a logit-normal
...
\item \textbf{Cross-organism, mechanism-blind example.} In a military
antimalarial--doxycycline cohort, overall resistance did not differ by exposure
while \emph{tet}(M) genes were significantly more common among the exposed --- a
single breakpoint concealing a mechanism shift.
\end{itemize}
...
public-health guidance only was assessed; clinic and provider-organization
...
Coding was blind to the expected outcome. A blinded second coder double-coded with
per-field Cohen's $\kappa$; agreement was 92\% and the single field with
meaningful disagreement ($\kappa=0.58$) is reported unrounded.
...
\textbf{Enumeration.} A closed universe of twelve established US governmental
systems (federal, state or metropolitan) that could plausibly capture, at a
population denominator, either doxy-PEP/doxycycline exposure or \sa{} AMR phenotype
was coded on three capabilities: exposure capture, phenotype capture (of the \sa{}
...
Tetracycline-resistant \sa{} is nationally notifiable nowhere. The full
system-by-system matrix with geographic resolution is in the repository.

\textbf{Dilution model.} The exposed-subgroup dilution fraction is
...
minimum detectable change in a proportion near baseline $R_0$ (two-sided
$\alpha=0.05$, 80\% power) inverts to
$\mathrm{RR_{needed}}=1+\mathrm{MDE}/(f R_0)$. Parameters were bounded to favor
detection, so a failed gate is robust: uptake $\in\{20,35,55\}\%$;
$\kappa\in\{0.2,0.5,1,3,5\}$ ($\kappa<1$ is the realistic region, as the exposed
are under-represented in a population isolate stream); $R_0\in\{5,10,13\}\%$;
isolate volume $N\in\{1\,000,\,10\,000,\,100\,000\}$ per geography-year ($100\,000$
exceeds any real US stream). Under the realistic cell,
$\mathrm{RR_{needed}}\approx14$--$64$ across the design-effect range; detection
appears only in a best-case cell (RR~$\approx$~1.93) that compounds four
simultaneously implausible assumptions and fails on any one alone. The metropolitan
break-even uses Washington, D.C. (2\,694 per 100\,000 adult males) as the observed
density ceiling and requires 2.1--33 times that density, or roughly 325\% of adult
males on PrEP under realistic isolate volumes (Figures~A--C).
...
uptake $\times$ isolate enrichment (best state, $R_0=10\%$, $N=100\,000$/state-year).
Every cell exceeds the optimistic benchmark RR~=~1.42.}
\end{figure}
...
\caption*{\textbf{Figure B. Metro level.} Male-PrEP density required for a
detectable shift, as multiples of the densest US geography (Washington, D.C.).
$1\times$ is already the national maximum.}
\end{figure}
\noindent Alt text: A chart showing required male-PrEP density in multiples of the
densest US metropolitan geography, with all requirements above the one-times
national maximum.
...
adult males on PrEP required for detection versus isolates per geography-year, by
...
\end{figure}
\noindent Alt text: Break-even curves of the share of all adult males on PrEP
required for detection against isolate volume, all lying above both the observed
2.7\% and the 100\% physical ceiling.
