% JAC v7 (2026-09-14) — compendium release v2.1.0 (supersedes v2.0.0, Zenodo 10.5281/zenodo.22725070; v2.1.0 DOI minted on deposit). US-centric refocus + UKHSA external comparator. Source repository: https://github.com/Nyx-Dynamics/doxypep-bystander
...
\usepackage{caption}
% Label tables as "Supplementary Table" throughout this standalone supplement.
...
streams instantiate the schema directly (Supplementary Table~\ref{tab:vector}).

\begin{table}[htbp]
\centering
\caption{Measurement dimensions required to follow the doxy-PEP resistance signal across the evidence chain.
The measurement vector comprises unit/population ($U$), exposure ($E$), phenotype/outcome ($P$), denominator ($D$), temporal structure ($T$), linkage ($L$), and measurement responsibility ($R$). ``Present'' indicates that the dimension is available at the resolution required by the downstream question; ``wrong unit/resolution'' indicates that the dimension exists but is not aligned with the target estimand; ``absent'' indicates that it is not captured.}
\label{tab:vector}
\small
...
The two critical gaps are $R$ in Stream B (no party assigned to measure the
...
do \emph{not} bear on the individual-level result the final trial
resolved.
...
\item \textbf{Mechanism-blind measurement in \sa{}.} In a
...
cohort is \emph{S. aureus}-specific (not cross-organism) (Mende \textit{et al.}, \textit{Diagn Microbiol
Infect Dis} 2016;86:211--220). The two determinants also differ in mobility:
\emph{tet}(M) is carried on conjugative transposons that cross strain boundaries,
whereas \emph{tet}(K) is typically plasmid-borne (Roberts and Mullany, \textit{FEMS
Microbiol Rev} 2011;35:856--871; Han \textit{et al.}, \textit{J Clin Microbiol}
2007;45:1350--1352), so the single breakpoint that merges the two phenotypes also
merges two different dissemination routes.
\end{itemize}
...
Tetracycline-resistant \sa{} is not nationally notifiable in the United States. Surveillance
...
Supplementary Table~\ref{tab:streamc}.

\begin{table}[htbp]
\centering
\caption{US surveillance and data systems assessed for doxy-PEP exposure and \emph{S. aureus} tetracycline phenotype.
Twelve systems were included because they could plausibly capture either exposure or antimicrobial-resistance phenotype at the required level; three additional systems or data sources were excluded with reasons shown. None of the 12 included systems captures both doxy-PEP exposure and \emph{S. aureus} tetracycline phenotype at a common population denominator.}
\label{tab:streamc}
\small
...
fraction is
\[f=\frac{(\text{male-PrEP}+\text{male-PLWH})\ \text{density}}{10^{5}}\ \text{uptake}\ d\ \kappa,\]
...
most favourable cell needs $\mathrm{RR_{needed}}$ 1.79. Against the more
conservative cross-organism benchmark (RR 1.42), the best metropolitan cell needs
about twice ($\approx$2$\times$) the combined male exposure density of the densest
US geography (Washington, D.C., 6\,406 per 100\,000 adult males in 2024), and
realistic cells hundreds of times it. All values regenerate from
...
\textbf{Dose-threshold sensitivity.} The dose-above-threshold share $d$ (the
fraction of users taking $>$3 doses/month, the subgroup carrying the benchmark
effect) enters $f$ linearly, so $\mathrm{RR_{needed}}-1$ scales as $1/d$; $d=1$ is
the limiting case in which \emph{every} user exceeds three doses/month. The
realistic-cell verdict holds across the plausible range and degrades only at that
bound, where the realistic lower bound (3.65) dips just below the trial's hazard
ratio (3.89) while still exceeding the cross-sectional benchmark band
(Supplementary Table~\ref{tab:dose}). Values regenerate via
\texttt{dilution\_plwh.dose\_sensitivity}.

\begin{table}[htbp]
\centering
\caption{Sensitivity of the modelled surveillance threshold to the proportion of doxy-PEP users exceeding three doses per month.
RR$_{\text{needed}}$ is shown under the primary combined PrEP+PLWH exposure specification. The primary analysis assumes $d=0.50$; $d=1.00$ is the limiting case in which every doxy-PEP user exceeds three doses per month. DEFF, design effect.}
\label{tab:dose}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
$d$ & realistic (DEFF 1) & realistic (DEFF 25) & best cell (single-comp) & best cell (DEFF 1)\\
\midrule
0.50 & 6.31 & 27.5 & 1.79 & 1.03\\
0.75 & 4.54 & 18.7 & 1.52 & 1.02\\
1.00 & 3.65 & 14.3 & 1.39 & 1.01\\
\bottomrule
\end{tabular}
\end{table}
...
\caption*{\textbf{Figure A. State-level relative risk required for a detectable change in \emph{S. aureus} tetracycline non-susceptibility.}
Modeled RR$_{\text{needed}}$ across doxy-PEP uptake and isolate-enrichment assumptions using the combined PrEP+PLWH exposure specification, the most favourable state exposure density, $R_0=10\%$, and 100\,000 isolates per state-year. Every cell exceeds the conservative cross-organism benchmark of RR 1.42; the \emph{S. aureus}-matched benchmark is RR 2.25.}
\end{figure}
\noindent Alt text: A heatmap grid of doxy-PEP uptake against isolate enrichment
in which every cell shows a required relative risk above the cross-organism
benchmark of 1.42.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth,keepaspectratio]{feasibility_metro.png}
\caption*{\textbf{Figure B. Metropolitan exposure density required for detection of a population-level resistance shift.}
Required combined PrEP+PLWH exposure density is expressed as a multiple of the densest observed US metropolitan geography, Washington, D.C. (6\,406 per 100\,000 adult males in 2024). Even under the most favourable modelled assumptions, detection requires approximately twice the highest observed exposure density.}
\end{figure}
...
\caption*{\textbf{Figure C. Break-even exposure prevalence required for population-level detection.}
Percentage of adult males who would need to be in the combined PrEP+PLWH exposed population for a detectable resistance shift, across isolate volume and enrichment assumptions. All modelled curves exceed the observed Washington, D.C. combined exposure density (6.4\%); under realistic isolate volumes, required exposure prevalence also exceeds the 100\% physical ceiling.}
\end{figure}
...
lying above the observed combined density (6.4\%), and above the 100\% physical
ceiling under realistic isolate volumes.
...
All analysis is Python 3.11+ (validated on CPython 3.12.12; pandas, statsmodels, scipy, matplotlib); no
