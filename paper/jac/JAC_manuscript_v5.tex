% JAC v5 (2026-09-13) — compendium release v2.1.0 (supersedes v2.0.0, Zenodo 10.5281/zenodo.22725070; v2.1.0 DOI minted on deposit). US-centric refocus + UKHSA external comparator. Source repository: https://github.com/Nyx-Dynamics/doxypep-bystander
...
\noindent Adrian C. \textsc{Demidont}*\\[0.6em]
...
in \sa{} and other commensals. One national programme has since taken up that
call: the UK Health Security Agency's 2026 doxy-PEP monitoring plan for England
...
conservative secondary. Because the trial's within-exposed hazard ratio (3.89)
sits well above this band, the detectability verdict below is robust to which
anchor is used: the relative risk a population system would need (RR~6--28,
Results) exceeds the cross-sectional benchmark across the realistic parameter
range, and exceeds the trial's own hazard ratio except at the limiting dose
assumption that every user takes $>$3 doses/month (dose-sensitivity in the
Supplementary data). Hazard ratio and risk ratio are in any case different
estimands, so this is a magnitude reference, not an equivalence.
...
the schema; a second, blinded LLM pass---a different interface (chat rather than
the command-line agent) but the same model family---independently re-coded a
5-document subsample (25 field-codings) for reliability, and disagreements were
...
generous: isolate volume was set generously high (favouring detection), while the
...
heterogeneity is itself a finding.
...
US jurisdictions) the result is identical: 0 instrument the bystander.
...
population denominator: no join is possible because one half does not exist.
...
\sa{} is nationally notifiable nowhere. A system could be built---most directly by
...
single-comparison inference; parameters in the Supplementary data). The verdict
turns on quantities no deployed system reports---exposure density, isolate
...
\caption{Schematic summary of the 12-system surveillance audit: no deployed US
system links doxy-PEP exposure to \sa{} tetracycline resistance at a common
population denominator. Exposure is captured only as a PrEP proxy (AIDSVu, CDC
AtlasPlus); the \sa{} tetracycline phenotype is captured at a population
denominator nowhere, because the one population-based platform reports the
methicillin (MRSA) subset. Of the 12 systems assessed, 0 join exposure to
phenotype.}
\label{fig:linkage}
\end{figure}
...
denominator; the more restrictive PrEP-only indexing gives 14--64). The trial HR
is shown as an empirical magnitude reference only; it is not numerically
interchangeable with the surveillance RR threshold because the estimands differ.
The signal is above the line while the systems built to follow it are not.}
\label{fig:detect}
\end{figure}
...
The three streams are summarised against the yardstick in Table~\ref{tab:audit}; a
...
before methicillin status is known; the two are orthogonal. So is the molecular
basis of tetracycline resistance: \emph{tet}(K) efflux leaves doxycycline largely
...
\emph{tet}(M)\cite{mcdougal2010}---a single breakpoint reporting one number for
two clinically opposite situations. An exposure cohort shows the same blindness:
doxycycline prophylaxis in 168 military personnel left overall tetracycline-class
resistance in \emph{S. aureus} unmoved while \emph{tet}(M) genes were significantly
more common in the exposed (p=0.031)\cite{mende2016}---a military antimalarial
cohort, not doxy-PEP. An inherited surveillance variable can stay valid for its
...
individuals, which fixes the exposed fraction and the denominator; sampling of the
oropharyngeal and peri-anal sites, since nares-only screening under-detects
colonisation by about 20\% in this population\cite{kyaw2012}; resistance reported
per carrier and by mechanism (\emph{tet}(M) versus \emph{tet}(K)) rather than as a
pooled tetracycline-class rate; dosing frequency rather than prescription status; a
population-denominator feed of \sa{} tetracycline susceptibility, most directly by
adding it to the platform that already tracks invasive MRSA and joining it to
exposure on geography; and access to individual-level trial data. None is exotic;
none is in place at scale.

England's national doxy-PEP monitoring plan is an instructive out-of-frame case:
postdating our audit and outside a US corpus, it nonetheless proposes the phenotype
...
architecture standardised around methicillin. We call this \emph{measurement
inheritance}: a downstream system retains the dimensions---estimand, denominator,
unit, cadence, linkage, measurement responsibility---appropriate to the upstream
question but not those the downstream question requires, so the question becomes
unanswerable not for want of data but for want of an inherited dimension. Built
from familiar parts but concerned with how they couple \emph{across} stages of an
evidence chain,\cite{tancredi2024surveillance,treem2023visibility} the downstream
gap is not closable by a better analysis of the trial's data---that analysis has
...
and mechanism remain open. Our coding carries two acknowledged asymmetries. The guidance corpus was
...
and archived at Zenodo (doi:10.5281/zenodo.22725070),\cite{demidont2026compendium}
