...
resistance, but whether the existing evidence base could detect it if it did. It is a
question in three parts, one for each place an answer would have to come from: the
clinical trials that measured the bystander, the guidelines built on them, and the
population surveillance meant to watch the intervention in the field. We ask of each
whether it is instrumented to register the signal the trials' own investigators
flagged for monitoring — whether, in short, the apparatus assembled around doxy-PEP can
see the one harm it was repeatedly told to look for.
